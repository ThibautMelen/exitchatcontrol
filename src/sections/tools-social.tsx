  ...
  return (
    <section id="ia">
      ...
      <Box tone="ok" label={<T fr="L'étape d'après : les agents" en="The next step: agents" />}>
        <p>
          <T
            fr={<>Le chatbot local règle la confidentialité de vos <em>questions</em> ; l'étape suivante, ce sont les <strong>agents</strong> — des IA qui agissent (fichiers, terminal, réseau). Là, le principe de ce guide s'applique au carré : ne pas faire confiance, <strong>vérifier avant d'exécuter</strong> — un plan relu avant le run, des permissions imposées par le moteur, des exécutions rejouables. Les options libres sont dans <a href="#dir-ia-locale-agentique">l'annuaire, catégorie IA locale &amp; agentique</a>.</>}
            en={<>A local chatbot fixes the privacy of your <em>questions</em>; the next step is <strong>agents</strong> — AI that acts (files, terminal, network). There, this guide's principle applies squared: don't trust, <strong>review before it runs</strong> — a plan reviewed before execution, permissions enforced by the engine, replayable runs. The free options sit in <a href="#dir-ia-locale-agentique">the directory, local &amp; agentic AI category</a>.</>}
          />
        </p>
      </Box>
    </section>
